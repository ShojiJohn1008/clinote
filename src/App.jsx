...
async function api(action, params = {}) {
  ...
  try {
    ...
    throw new Error("レスポンスのパースに失敗: " + text.slice(0, 100));
  }
}
...
function ReactionBar({ reactions, postId, onReact }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
      <style>{`@keyframes emojiPop{0%{transform:scale(1)}40%{transform:scale(1.5)}70%{transform:scale(.88)}100%{transform:scale(1)}}`}</style>
      {REACTION_OPTIONS.map(({ emoji, label }) => {
        ...
        return (
          <button key={emoji} onClick={(e) => { e.stopPropagation(); handleReact(emoji); }} style={{
            ...
            <span style={{ fontSize: 13, display: "inline-block", animation: animating[emoji] ? "emojiPop .3s ease-out" : "none" }}>{emoji}</span>
            {count > 0 && <span>{count}</span>}
            <span style={{ fontSize: 11, color: active ? "rgba(255,255,255,.8)" : "#9CA3AF" }}>{label}</span>
          </button>
        );
      })}
    </div>
...
function PostCard({ post, onClick, onReact }) {
  ...
  return (
    <div onClick={onClick} style={{ background: "white", borderRadius: 16, padding: 16, cursor: "pointer", border: "1px solid #F0F0F0", boxShadow: "0 1px 4px rgba(0,0,0,.04)" }}>
      <div style={{ display: "flex", gap: 10 }}>
        <div style={{ width: 9, height: 9, borderRadius: "50%", background: s.color, marginTop: 5, flexShrink: 0 }} />
        <div style={{ flex: 1 }}>
          <p style={{ margin: "0 0 10px", fontSize: 14, lineHeight: 1.65, color: "#1F2937", wordBreak: "break-all", overflowWrap: "break-word" }}>{post.content}</p>
...
function SetupScreen({ onComplete }) {
  ...
  const handleNameSubmit = async () => {
    ...
    try {
      const data = await api("login", { name: name.trim() });
      if (data.error) throw new Error(data.error);
      setUserId(data.userId);
      // すでにチームに入っている場合はスキップ
      if (data.teamId) {
        onComplete({ userId: data.userId, name: data.name, teamId: data.teamId });
      } else {
        setStep("team");
      }
    } catch (e) {
  ...
  const handleJoin = async () => {
    ...
    setLoading(true); setError("");
    try {
      const data = await api("joinTeam", { inviteCode: inviteCode.trim(), userId });
      if (data.error) throw new Error(data.error);
      onComplete({ userId, name, teamId: data.teamId, teamName: data.teamName });
    } catch (e) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const handleCreate = async () => {
    ...
    setLoading(true); setError("");
    try {
      const data = await api("createTeam", { teamName: teamName.trim(), userId });
      if (data.error) throw new Error(data.error);
      onComplete({ userId, name, teamId: data.teamId, teamName: data.teamName, inviteCode: data.inviteCode });
    } catch (e) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const inputStyle = { width: "100%", border: "1px solid #E5E7EB", borderRadius: 12, padding: "12px 14px", fontSize: 15, outline: "none", boxSizing: "border-box", marginBottom: 12, fontFamily: "inherit" };
  const btnStyle = (active) => ({ width: "100%", padding: "13px 0", background: active ? "#111827" : "#E5E7EB", color: active ? "white" : "#9CA3AF", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: active ? "pointer" : "default" });
      ...
      <div style={{ width: "100%", maxWidth: 360 }}>
        ...
        {step === "name" && (
          <div style={{ background: "white", borderRadius: 20, padding: 24, border: "1px solid #F0F0F0" }}>
            <p style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 600, color: "#111827" }}>あなたの名前を入力</p>
            <input value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === "Enter" && handleNameSubmit()} placeholder="例：山田 太郎" style={inputStyle} />
            {error && <p style={{ color: "#EF4444", fontSize: 13, margin: "0 0 10px" }}>{error}</p>}
            <button onClick={handleNameSubmit} disabled={!name.trim() || loading} style={btnStyle(name.trim() && !loading)}>
              {loading ? "接続中..." : "次へ →"}
            </button>
          </div>
        )}
        ...
        {step === "team" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ background: "white", borderRadius: 20, padding: 24, border: "1px solid #F0F0F0" }}>
              ...
              <input value={inviteCode} onChange={e => setInviteCode(e.target.value.toUpperCase())} placeholder="例：ABC123" style={{ ...inputStyle, letterSpacing: 2 }} />
              <button onClick={handleJoin} disabled={!inviteCode.trim() || loading} style={btnStyle(inviteCode.trim() && !loading)}>
                {loading ? "処理中..." : "参加する"}
              </button>
            </div>
            ...
            <div style={{ background: "white", borderRadius: 20, padding: 24, border: "1px solid #F0F0F0" }}>
              ...
              <input value={teamName} onChange={e => setTeamName(e.target.value)} placeholder="例：三重大学総合診療科" style={inputStyle} />
              <button onClick={handleCreate} disabled={!teamName.trim() || loading} style={{ ...btnStyle(teamName.trim() && !loading), background: teamName.trim() ? "#3B82F6" : "#E5E7EB" }}>
                {loading ? "処理中..." : "チームを作成"}
              </button>
            </div>
...
export default function App() {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await api("getPosts", { type: tab, userId: user.userId });
      if (data.error) throw new Error(data.error);
      setPosts(data.posts || []);
  ...
  const openPost = async (post) => {
    setActivePost(post); setScreen("detail"); setComments([]);
    try {
      const data = await api("getComments", { postId: post.id });
  ...
  const handleSubmitPost = async () => {
    ...
    try {
      ...
      if (data.error) throw new Error(data.error);
      setPostContent("");
      setTab(postType); // 投稿したタブ（疑問/学び）に合わせる
      setPostType("question");
      setPostStatus("dot");
      setScreen("home");
      // setScreen("home")でuseEffectが発火しfetchPostsが呼ばれる
      // 直接fetchPostsを呼ぶと二重になりタイミングがずれるため呼ばない
  ...
  const handleReact = async (postId, emoji, adding) => {
    ...
    try { await api("toggleReaction", { postId, emoji, userId: user.userId }); }
    catch (e) { console.error(e); }
  };
  ...
  const handleTouchEnd = (e) => {
    ...
    if (Math.abs(dx) > 50) { if (dx < 0 && tab === "question") setTab("learning"); if (dx > 0 && tab === "learning") setTab("question"); }
  ...
  return (
    <div style={{ fontFamily: "'Hiragino Sans','Noto Sans JP',sans-serif", background: "#F8F9FB", minHeight: "100vh", maxWidth: 390, margin: "0 auto", display: "flex", flexDirection: "column", position: "relative" }}>
      ...
      <div style={{ background: "white", borderBottom: "1px solid #F0F0F0", padding: "16px 20px 12px", display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, zIndex: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          {screen === "home" && <div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ width: 28, height: 28, borderRadius: 8, background: "linear-gradient(135deg,#F97316,#3B82F6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>💡</div><span style={{ fontWeight: 700, fontSize: 17, color: "#111827" }}>Clinote</span></div>}
          ...
          <div style={{ background: "white", padding: "0 20px", borderBottom: "1px solid #F0F0F0", flexShrink: 0 }}>
            <div style={{ display: "flex" }}>
              {[["question", "疑問"], ["learning", "学び"]].map(([key, label]) => (
                <button key={key} onClick={() => setTab(key)} style={{ flex: 1, padding: "14px 0", border: "none", background: "none", fontWeight: tab === key ? 700 : 400, color: tab === key ? "#111827" : "#9CA3AF", fontSize: 15, cursor: "pointer", borderBottom: tab === key ? "2.5px solid #111827" : "2.5px solid transparent", transition: "color .15s,border-color .15s" }}>{label}</button>
              ))}
            </div>
          </div>
            ...
            <div style={{ marginLeft: 20, borderLeft: "2px solid #E5E7EB", paddingLeft: 16, display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {comments.map(c => (
                <div key={c.id} style={{ background: "white", borderRadius: 14, padding: 14, border: "1px solid #F0F0F0" }}>
                  ...
                  <p style={{ margin: 0, fontSize: 14, lineHeight: 1.65, color: "#374151", wordBreak: "break-all", overflowWrap: "break-word" }}>{c.content}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {screen === "post" && (
        <div style={{ flex: 1, padding: 16, display: "flex", flexDirection: "column", gap: 16, overflowY: "auto", paddingBottom: 80 }}>
          <div style={{ background: "white", borderRadius: 14, padding: 4, display: "flex", border: "1px solid #F0F0F0" }}>
            {[["question", "疑問"], ["learning", "学び"]].map(([key, label]) => (
              <button key={key} onClick={() => setPostType(key)} style={{ flex: 1, padding: "10px 0", border: "none", cursor: "pointer", borderRadius: 10, fontSize: 14, fontWeight: 600, transition: "all .15s", background: postType === key ? "#111827" : "transparent", color: postType === key ? "white" : "#9CA3AF" }}>{label}</button>
            ))}
          </div>
            ...
            <div style={{ background: "white", borderRadius: 14, padding: 16, border: "1px solid #F0F0F0" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {[["dot", "🟠 調べた"], ["half", "🟣 途中"], ["line", "🔵 理解した"]].map(([key, label]) => (
                  <button key={key} onClick={() => setPostStatus(key)} style={{ flex: 1, padding: "8px 4px", border: `1px solid ${postStatus === key ? statusConfig[key].color : "#E5E7EB"}`, borderRadius: 10, fontSize: 11, fontWeight: 600, cursor: "pointer", background: postStatus === key ? statusConfig[key].bg : "white", color: postStatus === key ? statusConfig[key].color : "#9CA3AF", transition: "all .15s" }}>{label}</button>
                ))}
              </div>
            </div>
          )}
          <div style={{ background: "white", borderRadius: 14, padding: "14px 16px", border: "1px solid #F0F0F0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <button onClick={() => setIsAnonymous(!isAnonymous)} style={{ width: 48, height: 28, borderRadius: 14, border: "none", cursor: "pointer", background: isAnonymous ? "#D1D5DB" : "#111827", position: "relative", transition: "background .25s" }}>
              <div style={{ position: "absolute", top: 3, left: isAnonymous ? 3 : 23, width: 22, height: 22, borderRadius: "50%", background: "white", transition: "left .25s", boxShadow: "0 1px 3px rgba(0,0,0,.2)" }} />
            </button>
          </div>
        </div>
      )}
